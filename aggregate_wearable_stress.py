...
OUTPUT
------
Two aggregated DataFrames (df_v1, df_v2) — one row per valid stress
observation — with columns:

  participant_id   str            e.g. "S01", "f07"
  phase            str            protocol phase label (Transitions, Pre- and
                                  Post-protocol are excluded entirely)
  timestamp        datetime64[ns] last ACC timestamp within the aggregation window
  <signal>_mean    float          mean of the signal within the window
  <signal>_sd      float          sample std dev (ddof=1); NaN when n < 2
  <signal>_max     float          maximum value within the window
  <signal>_min     float          minimum value within the window
                   signals: ACC_x, ACC_y, ACC_z, BVP, EDA, TEMP, HR, IBI
  reported_stress  float          self-reported stress score (same value for
                                  all windows in the same phase)
  weight           float          1 / n_windows for that phase; sums to 1.0
                                  within each (participant, phase) pair

WINDOWING STRATEGY
------------------
Each phase is divided into non-overlapping 30-second windows, stepped
backward from phase_end.  Signals are aggregated at their native sampling
rates within each window — no resampling.

  Window k=0 (always included, even for short phases):
      start = max(phase_end - 30 s, phase_start)  [clamped for short phases]
      end   = phase_end

  Window k>0 (included only when the full 30 s fits within the phase):
      start = phase_end - (k+1) * 30 s
      end   = phase_end - k * 30 s
      condition: start >= phase_start

  Leading fraction that does not fit a full 30 s window is discarded.

Examples:
  Phase =  27 s  → 1 window:  [phase_start, phase_end]  (shortened, 27 s)
  Phase =  32 s  → 1 window:  [phase_end-30s, phase_end]  (30 s)
  Phase =  75 s  → 2 windows: [45s, 75s],  [15s, 45s]   (first 15s discarded)
...
def compute_windows(
    ...
    """
    Compute non-overlapping 30-second windows for one phase, stepping backward
    from phase_end.

    Always returns at least one window (the last window is included even when
    the phase is shorter than 30 seconds; its start is clamped to phase_start).

    Window k=0 (always):
        start = max(phase_end - 30s, phase_start)
        end   = phase_end

    Window k>0 (only if start >= phase_start):
        start = phase_end - (k+1)*30s
        end   = phase_end - k*30s

    Returns list ordered closest-to-phase_end first.
    """
    windows = []
    k = 0
    while True:
        win_end   = phase_end - k * WINDOW_TD
        win_start = phase_end - (k + 1) * WINDOW_TD

        if k == 0:
            win_start = max(win_start, phase_start)   # clamp for short phases
            windows.append((win_start, win_end))
        else:
            if win_start >= phase_start:
                windows.append((win_start, win_end))
            else:
                break   # remaining lead < 30 s, discard

        k += 1

    # Reverse so the list is in chronological order (earliest window first)
...
def _slice_signal(
    ...
    """
    Return rows where win_start <= timestamp < win_end.
    Half-open interval keeps adjacent windows non-overlapping.
    """
    ts = df["timestamp"]
    return df.loc[(ts >= win_start) & (ts < win_end)]
...
def aggregate_window(
    ...
    """
    Aggregate all signals within one (win_start, win_end) window.

    Each signal is sliced at its native rate (no resampling).
    The window's representative timestamp is the last ACC timestamp within the
    window, falling back to win_end if ACC is unavailable.
    ...
    if "IBI" in signals and not signals["IBI"].empty:
        ibi_slice = _slice_signal(signals["IBI"], win_start, win_end)
...
def build_aggregated_rows(
    ...
    """
    For each non-skipped phase, compute windowed aggregates.
    ...
    Returns
    -------
    list of row dicts, one per aggregation window
    """
    rows: list[dict] = []
    ...
    for phase_start, phase_end, phase_label in boundaries:
        ...
        windows   = compute_windows(phase_start, phase_end)
        n_windows = len(windows)
        score     = stress_lut.get(phase_label, np.nan)
        weight    = 1.0 / n_windows          # weights sum to 1.0 within phase

        for win_start, win_end in windows:
            row = aggregate_window(signals, win_start, win_end)
            row["participant_id"]  = participant_id
            row["phase"]           = phase_label
            row["reported_stress"] = score
            row["weight"]          = weight
            rows.append(row)
...
def main() -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    tail_cols = ["reported_stress", "weight"]
    ...
    for label, df in [("df_v1", df_v1), ("df_v2", df_v2)]:
        win_stats = df.groupby("participant_id").size().describe()
        ...
        print(f"           Windows per participant — "
              f"min={int(win_stats['min'])}, "
              f"median={int(win_stats['50%'])}, "
              f"max={int(win_stats['max'])}")
    print("=" * 60)
