...
def make_sequence(pid_df, signal=None):
    ...
    w : (T,)  weight of prediction target
    T = len(pid_df) - 1
    """
    d      = pid_df.reset_index(drop=True)
    stress = d[TARGET].values.astype(np.float32)
    weight = d[WEIGHT_COL].values.astype(np.float32)
...
def apply_scaler(seqs, mu, std):
    return [((x - mu) / std, y, w) for x, y, w in seqs]
...
def apply_scaler_y(seqs, mu_y, std_y):
    """Normalise y; leave x and w unchanged."""
    return [(x, (y - mu_y) / std_y, w) for x, y, w in seqs]
...
def train_model(train_seqs, val_seqs, hidden_size, noise_sigma):
    ...
    for epoch in range(MAX_EPOCHS):
        ...
        for x, y, w in train_seqs:
            xt = torch.tensor(x).unsqueeze(0)
            yt = torch.tensor(y).unsqueeze(0)
            wt = torch.tensor(w).unsqueeze(0)
            opt.zero_grad()
            wmse(model(xt, training=True), yt, wt).backward()
        ...
        with torch.no_grad():
            val_loss = np.mean([
                wmse(model(torch.tensor(x).unsqueeze(0)),
                     torch.tensor(y).unsqueeze(0),
                     torch.tensor(w).unsqueeze(0)).item()
                for x, y, w in val_seqs
            ])
...
def eval_model(model, seqs, mu_y=0.0, std_y=1.0):
    """
    Compute WMSE in the *original* stress scale by inverse-transforming
    ...
    with torch.no_grad():
        ...
        for x, y, w in seqs:
            ...
            losses.append(wmse(pt, yt, wt).item())
...
def hp_search(train_seqs, k_inner=3):
    ...
    Average val WMSE across folds for each HP combo.
    """
...
# ── K-Fold (participant-level) ────────────────────────────
def make_kfold_splits(participants, k=4):
    """
    Shuffle participants once, split into k roughly equal groups.
    Each group becomes a test fold; no participant is ever split.
    """
    rng   = random.Random(SEED)
    pids  = participants[:]
    rng.shuffle(pids)
    folds = [pids[i::k] for i in range(k)]   # round-robin assignment → balanced sizes
    return folds

def run_kfold(df, participants, signal, k=4):
    label        = signal if signal else 'stress_only'
    fold_results = []
    folds        = make_kfold_splits(participants, k)

    for fold_idx, test_pids in enumerate(folds):
        train_pids = [p for p in participants if p not in test_pids]
        print(f"  Fold {fold_idx+1}/{k} | test={test_pids}", end='  ')
        ...
        test_wmse = eval_model(model, test_sc_x, mu_y, std_y)

        print(f"hidden={best_hidden} σ={best_sigma:.2f} WMSE={test_wmse:.4f}")
        fold_results.append({
            'fold'            : fold_idx + 1,
            'test_participants': str(test_pids),
            'signal'          : label,
...
if __name__ == '__main__':
    ...
    all_folds += run_kfold(df, pids, signal=None)

    for sig in SIGNALS:
        print(f"\n── stress + {sig} ──")
        all_folds += run_kfold(df, pids, signal=sig)
    ...
    baseline = (df_folds[df_folds['signal']=='stress_only']
                [['fold','test_wmse']]
                .rename(columns={'test_wmse':'wmse_restricted'}))

    rows = []
    for sig in SIGNALS:
        sub = (df_folds[df_folds['signal']==sig]
               [['fold','test_wmse','hidden_size','noise_sigma']]
               .merge(baseline, on='fold'))
        sub['impr_%'] = 100 * (sub['wmse_restricted'] - sub['test_wmse']) / sub['wmse_restricted']
        rows.append({
            'signal'            : sig,
            'mean_wmse_restr'   : sub['wmse_restricted'].mean(),
            'mean_wmse_unrest'  : sub['test_wmse'].mean(),
    ...
    print("\n" + "=" * 60)
    print("SUMMARY (sorted by out-of-sample WMSE improvement %)")
    print("=" * 60)
