...
Loss          : Weighted MSE  →  Σ w_i*(y_i - ŷ_i)² / Σ w_i
Evaluation    : Leave-One-Participant-Out Cross-Validation (18 folds)
Granger test  : F-test on training residuals (RSS_R - RSS_U) + out-of-sample WMSE drop
"""
...
def build_features(
    ...
    w = df_r['__w__'].values.astype(float)
    return X, y, w, feat_cols
...
def pool_participants(
    ...
    for pid in participants:
        ...
        X, y, w, feat_names = build_features(pid_df, lag, signal)
    ...
    return X_pooled, np.concatenate(ys), np.concatenate(ws), names

# ─────────────────────────────────────────────
# WLS fitting
# ─────────────────────────────────────────────
def fit_wls(X: np.ndarray, y: np.ndarray, w: np.ndarray) -> sm.regression.linear_model.RegressionResultsWrapper:
    Xc = sm.add_constant(X, has_constant='add')
    return sm.WLS(y, Xc, weights=w).fit()

def predict_wls(model, X: np.ndarray) -> np.ndarray:
    Xc = sm.add_constant(X, has_constant='add')
...
def select_lag_aic(
    ...
    for lag in range(1, max_lag + 1):
        X, y, w, _ = pool_participants(df, train_pids, lag, signal, noise_std=noise_std)
...
def granger_f_test(
    ...
    resid_r = y_r - predict_wls(model_r, X_r)
    resid_u = y_u - predict_wls(model_u, X_u)

    rss_r = float(np.sum(w_r * resid_r ** 2))
    rss_u = float(np.sum(w_u * resid_u ** 2))
...
def run_loocv(
    ...
    for i, test_pid in enumerate(participants):
        ...
        X_u_tr, y_u_tr, w_u_tr, feat_u = pool_participants(df, train_pids, best_lag, signal, noise_std=noise_std)
        X_r_tr, y_r_tr, w_r_tr, feat_r = pool_participants(df, train_pids, best_lag, None, noise_std=noise_std)

        if X_u_tr is None or X_r_tr is None:
            continue

        model_u_tr = fit_wls(X_u_tr, y_u_tr, w_u_tr)
        model_r_tr = fit_wls(X_r_tr, y_r_tr, w_r_tr)

        if signal is not None:
            F_stat, p_val = granger_f_test(
                model_r_tr, model_u_tr,
                X_r_tr, y_r_tr, w_r_tr,
                X_u_tr, y_u_tr, w_u_tr
            )
        else:
        ...
        X_u_te, y_te, w_te, _ = build_features(test_df_pid, best_lag, signal)
        X_r_te, _,    _,   _  = build_features(test_df_pid, best_lag, None)

        if X_u_te is None or len(X_u_te) == 0:
            continue

        y_pred_u = predict_wls(model_u_tr, X_u_te)
        y_pred_r = predict_wls(model_r_tr, X_r_te)

        wmse_u = weighted_mse(y_te, y_pred_u, w_te)
        wmse_r = weighted_mse(y_te, y_pred_r, w_te)
        ...
        fold_results.append({
            ...
            'F_stat_train'     : F_stat,
            'p_val_train'      : p_val,
            'wmse_unrestricted': wmse_u,
            'wmse_restricted'  : wmse_r,
            'wmse_improvement' : wmse_r - wmse_u,
            'wmse_improvement_%': 100 * (wmse_r - wmse_u) / wmse_r if wmse_r > 0 else np.nan,
            'n_test'           : len(y_te)
        })
...
def summarise(all_folds: List[Dict]) -> pd.DataFrame:
    ...
    for (noise_val, sig), grp in df_folds.groupby(['noise_std', 'signal']):
        ...
        row = {
            ...
            'mean_wmse_restricted'   : grp['wmse_restricted'].mean(),
            'mean_wmse_unrestricted' : grp['wmse_unrestricted'].mean(),
            'mean_wmse_improvement'  : grp['wmse_improvement'].mean(),
            'mean_wmse_improvement_%': grp['wmse_improvement_%'].mean(),
            'folds_u_better'         : (grp['wmse_improvement'] > 0).sum(),
        }
        rows.append(row)

    summary = pd.DataFrame(rows).sort_values(by=['noise_std', 'mean_wmse_improvement_%'], ascending=[True, False])
    return summary
...
if __name__ == '__main__':
    ...
    display_cols = [
        ...
        'folds_p<0.05', 'mean_wmse_restricted', 'mean_wmse_unrestricted',
        'mean_wmse_improvement_%', 'folds_u_better'
    ]
